...
size_t dos_low_level_seek_relative(int fd, int32_t pos)
{
    off_t new_pos;
...
FILE *dos_fopen(const char *path, const char *flags)
{
    char native_path[FILENAME_MAX];
...
/*
 * On DOS, this function sets the mode of the file descriptor to
 * O_BINARY or O_TEXT.
 */
int dos_setmode(int fd, int mode)
{
    return 0;
}
...
static bool find_file_in_directory(const char *directory,
  const char *filename, char *buffer, size_t size)
{
    DIR *dir;
    struct dirent *de;
    bool found = false;
...
static bool lookup_path(const char *path, char *buffer, size_t size)
{
      ...
      || strcmp(segment, FS_SEP_STR) == 0) /* absolute path */
    {
      ...
      if (!find_file_in_directory(prefix, segment, exsegment,
        sizeof(exsegment)))
      {
    ...
    {
        if (!find_file_in_directory(".", segment, exsegment,
          sizeof(exsegment)))
        {
            snprintf(buffer, size, "%s", path);
            strtolower(buffer);
    ...
    for (;;)
    {
          ...
          sizeof(exsegment)))
        {
            ...
            if (rest != NULL)
            {
                ...
                snprintf(buffer, size, "%s" FS_SEP_STR "%s%s",
                  prefix, segment, exsegment);
            }
            else
                snprintf(buffer, size, "%s" FS_SEP_STR "%s",
                  prefix, segment);
...
bool dos_path_to_native(const char *path, char *buffer, size_t size)
{
